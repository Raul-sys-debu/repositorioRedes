...
if __name__ == "__main__":  # Si este archivo se ejecuta directamente.
    while True:  # Bucle infinito para mostrar el menú.
        ...
        opcion = input("\nSelecciona una opción (1-4): ").strip()  # Solicita una opción al usuario.
 
        if opcion == "1":  # Si se selecciona la opción de escaneo de puertos.
            host = input("Ingresa la IP del host a escanear: ").strip()  # Solicita la IP del host.
            if not validar_ip(host):  # Verifica si la IP es válida.
                print("[!] IP inválida.")  # Si la IP es inválida, muestra un mensaje.
                continue  # Vuelve a solicitar la IP.
 
            rango = input("¿Qué rango de puertos deseas escanear? (Ej: 1-1024 o presiona Enter para todos): ").strip()  # Solicita el rango de puertos.
            if rango:  # Si se ingresa un rango.
                try:
                    start, end = map(int, rango.split('-'))  # Convierte el rango en un par de enteros.
                    puertos = range(start, end + 1)  # Crea el rango de puertos a escanear.
                except:
                    print("[!] Rango inválido.")  # Si el rango no es válido, muestra un mensaje.
                    continue  # Vuelve a solicitar el rango.
            else:
                puertos = range(1, 65536)  # Si no se ingresa un rango, se escanean todos los puertos.
 
            port_scan(host, puertos)  # Llama a la función de escaneo de puertos.
 
        elif opcion == "2":  # Si se selecciona la opción de Ping Sweep.
            subred = input("Ingresa la subred (Ej: 192.168.1.0/24): ").strip()  # Solicita la subred.
            ping_sweep(subred)  # Llama a la función de Ping Sweep.
 
        elif opcion == "3":  # Si se selecciona la opción de detección de SO remoto.
            host = input("Ingresa la IP del host a analizar: ").strip()  # Solicita la IP del host.
            fingerprint_os(host)  # Llama a la función de detección de SO.
 
        elif opcion == "4":  # Si se selecciona la opción para salir.
            print("Saliendo... ¡Hasta luego!")  # Muestra un mensaje de despedida.
            break  # Sale del bucle y termina el programa.
        else:
            print("[!] Opción no válida.")  # Si la opción ingresada no es válida, muestra un mensaje.
